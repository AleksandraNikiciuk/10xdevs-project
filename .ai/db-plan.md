...
| `user_id`       | `uuid`          | `NOT NULL`, `FOREIGN KEY (users.id)`                               | Denormalized reference to the user for easy RLS and queries. |
...
RLS will be enabled on all tables containing user data to ensure data privacy.
